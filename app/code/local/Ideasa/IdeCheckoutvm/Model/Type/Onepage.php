...
class Ideasa_IdeCheckoutvm_Model_Type_Onepage extends Mage_Core_Model_Abstract {
  ...
  public function getQuote() {
    if (!$this->getCheckout()->hasQuote()) {
      //throw new Exception("Sorry, no quotes are available for this order at this time.");
    }
  ...
  public function setDefaultShippingMethod(Mage_Sales_Model_Quote $quote) {
    ...
    if (empty($defaultShippingMethod)) {
			if($this->logger->isDebugEnabled()) {
				$this->logger->debug('Método de Entrega default não configurado.');
			}
    }
