        ...
        if ($email && $email != '') {
          if (!$validator->isValid($email)) {
            ...
            if ($this->_isEmailRegistered($email)) {
              ...
              $msg = Mage::helper('checkout')->__('There is already a customer registered using this email address. Please login using this email address or enter a different email address to register your account.');
              $result->setErrorMessage(Ideasa_IdeCheckoutvm_ErrorMessage::getInstance($msg));
            } else {
              $result->setSuccess(true);
            }
          }
